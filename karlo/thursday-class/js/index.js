...
async function runEverything() {
  ...
  try {
    ...
    console.log('my_error came from a catch');
...
async function canIAffordIt() {

  console.log(`Running: canIAffordIt()`);

  let retrieved_data = await getDataFromInternet();

  console.log(`retrieved_data`);console.log(retrieved_data);

  let retrieved_products = retrieved_data['products'];
  let retrieved_users = retrieved_data.users;

  let picked_product = randomlyPickProduct(retrieved_products);
  let picked_product_price = picked_product['price'];

  let first_user = retrieved_users[0];
  let first_user_money = first_user['money'];

  if (isNaN(first_user_money)) {
    throw new Error(`We don't know our amount of money!`);
  }

  console.log(`Comparing money (${first_user_money}) to price (${picked_product_price})`);

  if (picked_product <= first_user_money) {

    console.log(`yay! I can afford it!`);
  }

  else {
    throw new Error(`I can't afford it! 😭`);
  }
}

function function_to_run_upon_successful_promise(result) {
  
  console.log('this should happen first! the result is...');
  console.log(result);
}
...
async function getDataFromInternet() {
  ...
  const my_promise = fetch("https://my-json-server.typicode.com/mrkiley/cwb2023-onlineshop/db");

  // console.log(my_promise);
  
  // my_promise.then(function_to_run_upon_successful_promise); // Asynchronous
  let result = await my_promise; // Synchronous

  // function_to_run_upon_successful_promise();

  // Extract the data from the retrieved source into the JSON format
  json_data = await result.json();

  return json_data;
}

function randomlyPickProduct(product_list) {

  let number_of_products = product_list.length;

  let random_index = Math.floor(Math.random() * number_of_products);

  let picked_product = product_list[random_index];
...
// Which products are lower or equal to my money